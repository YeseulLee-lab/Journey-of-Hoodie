...
public class TalkManager : MonoBehaviour
{
    ...
    void Awake()
    {
      ...
      // nickname = StageManager.GetComponent<TransData>().nickname;
        ...
        if (null != StageManager){
            nickname = StageManager.GetComponent<TransData>().nickname;
            icenickname = StageManager.GetComponent<TransData>().icenickname;
        }
        else{
            nickname = "플레이어";
            icenickname = "아이스";
    ...
    void GenerateData()
    {
        ...
        talkData.Add(10000, new string[] { icenickname + "아(야)! 정신 차려! 우리 행성이 파괴당하고 있다고!:6", "뭐야 이 자식 세뇌풀렸네?:0", "어? 뭐야 눈 앞이 이상해:0", 
        "으아악! 내 몸을 돌려내!!:7", icenickname + "아(야)! 정신 차려야 돼! 우리 행성을 구해야지!:6", "구하긴 뭘 구해! 이미 너희 행성은 끝났다고! 정신을 차린다고 구할 수 있을 거 같아?:0"});
